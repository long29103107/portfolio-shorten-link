...
public sealed record ActivateShortLinkCommand(
    string Code) : IRequest<ShortLinkDeactivatedResponse>;

internal sealed class ActivateShortLinkCommandHandler(
    IShortLinkService shortLinkService,
    ShortLinkAccessGuard accessGuard,
    ShortLinkAuditWriter auditWriter)
    : IRequestHandler<ActivateShortLinkCommand, ShortLinkDeactivatedResponse>
{
    public async Task<ShortLinkDeactivatedResponse> Handle(
        ActivateShortLinkCommand request,
        CancellationToken cancellationToken)
    {
        ...
        ShortLinkFeatureSupport.EnsureSucceeded(
            await shortLinkService.ActivateAsync(request.Code, cancellationToken));
        await auditWriter.RecordAsync(
            ...
            cancellationToken: cancellationToken);
        return new ShortLinkDeactivatedResponse(request.Code, true);
    }
}
