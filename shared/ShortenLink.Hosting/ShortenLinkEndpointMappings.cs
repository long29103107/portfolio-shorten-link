...
public static class ShortenLinkEndpointMappings
{
    ...
    private static Task<ShortLinkAdminListResponse> ListShortLinksAsync(
        ...
        string? sortBy, string? sortDirection, ISender sender, IOptions<ShortenLinkOptions> options,
        HttpContext httpContext, CancellationToken cancellationToken) =>
        sender.Send(new ListShortLinksQuery(
            GetBaseUrl(options.Value, httpContext), limit, page, cursor, search, status, sortBy, sortDirection),
            cancellationToken);
